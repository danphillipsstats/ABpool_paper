...
name <- "v20_cox_regression_nobs_propmis_20_scen_Bayesimp_cens20_lognormalXskew2_B5000"
...
exponential_model = rstan::stan_model(file=paste0(program_directory,'/Lognormal_covariate_exponential_model.stan'))
...
# sim_cox_pow(20,0.8,0.2,basehaz=1)
# candidate_params <- list(list("X_eff" = seq(from=0.88,0.91,by=0.005), "n" = 20),
#                          list("X_eff" = seq(from=0.468,0.476,by=0.002), "n" = 50),
#                          list("X_eff" = seq(from=0.312,0.32,by=0.002), "n" = 100),
#                          list("X_eff" = seq(from=0.216,0.22,by=0.001), "n" = 200))
#                          #list("X_eff" = 0.141, "n" = 500),
#                          #list("X_eff" = 0.099, "n" = 1000))
...
#               "param" = params))
# }
# power_list4 <- lapply(candidate_params,power_vec, B = 10000)
...
#   }
# }
# plot_power_list(power_list4)

n_obss <- c(20,50,100,200) # 20,50,100,200,500
print(n_obss)
betas <- c(0.894,0.471,0.315,0.217) # 0.872,0.475,0.324,0.225,0.141
...
cox_regression_sims <- function(B, parameters_in){
  ...
  mean_ln <- exp(0.5*ln_sigma^2)
  sd_ln <- sqrt(exp(ln_sigma^2)*(exp(ln_sigma^2)-1))
  ...
  while (count <= B){
    ...
    X_true <- (rlnorm(n, meanlog = 0, sdlog = ln_sigma)-mean_ln)/sd_ln # lognormal covariate minus mean, then divided by sd - note after standardising, meanlog cancels out, so we can use meanlog = 0 wlog
    if (n%%2==0){
    ...
    if(!inherits(cox.out.full, "error")){
      ...
      res <- tryCatch({
        stan_obj = rstan::sampling(
          ...
          pars = c("std_log_X_mis_unstd","mean_ln","sd_ln"),
          include = FALSE,
          control = list(adapt_delta = 0.97, max_treedepth = 15),
          refresh = sampling_iterations/10)
        stan_obj
      }, error = function(e) {
        saveRDS(list(error=conditionMessage(e), session=sessionInfo()), file = sprintf("error_pid%s_iter%s.rds", Sys.getpid(), count))
        stop(e)
      })
      t_stan_2 <- Sys.time()
...
# out_test <- cox_regression_sims(2,parameters_list[[1]])
...
for (i in 1:length(parameters_list)){
  ...
  snowfall::sfExport("B_seq","m","results","results_i",
                     "sample_ABpool_norm","sample_ABpool_t_Cox","program_directory",
                     "skewness","kurtosis")
