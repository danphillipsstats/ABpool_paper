...
name <- "v20_linear_regression_nobs_propmis_25_scen_Bayesimp_B5000"

require(mice)
require(parallel)
require(abind)

linear_model = rstan::stan_model(file=paste0(program_directory,'/Normal_covariate_linear_model.stan'))
...
# }
# linear_regression_power_calc(n=50,Z_eff=0.2,
#                              X_eff=0.425,B_small = 10000)
# # 
# candidate_params <- list(list("X_eff" = seq(1.284,1.286,0.001),"n" = 10,"B_small"=50000),
#                             list("X_eff" = seq(0.733,0.736,0.001), "n" = 20,"B_small"=50000),
#                             list("X_eff" = seq(0.42,0.422,0.001), "n" = 50,"B_small"=50000),
#                             list("X_eff" = seq(0.286,0.288,0.001), "n" = 100,"B_small"=50000),
#    list("X_eff" = seq(0.20,0.202,0.001), "n" = 200,"B_small"=10000))
...
#     pow[i] <- linear_regression_power_calc(n,Z_eff = Z_eff,X_eff = X_eff,B_small=B_small)
#   }
#   return(list("power" =pow,
#               "param" = params))
# }
# power_list9 <- parallel::mclapply(candidate_params,power_vec)
# power_list9
...
set.seed(12345)                         # master seed for reproducibility
...
# test <- linear_regression_sims(2,parameters_list[[length(parameters_list)]])
# parameters_in <- parameters_list[[length(parameters_list)]]
# parameters_list <- parameters_list[c(length(parameters_list))]
...
for (i in 1:length(parameters_list)){
  ...
  snowfall::sfExport("B_seq","m","results","results_i",
                     "sample_ABpool_norm","sample_ABpool_t","linear_model",
                     "skewness","kurtosis")
  ...
  rownames(out$other_info_summary)[1] <- "mean"
