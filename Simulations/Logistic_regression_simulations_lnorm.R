...
name <- "v21_logistic_regression_nobs_propmis_15_scen_Bayesimp_lognormalXskew2_B5000"

require(mice)
require(parallel)
require(abind)

logistic_model = rstan::stan_model(file=paste0(program_directory,'/Lognormal_covariate_logistic_regression.stan'))
...
logistic_regression_power_calc <- function(n,Z_eff,X_eff,B_small = 1000){
  ...
  error_count <- 0
  while (count <= B_small){
    X <- (rlnorm(n,sd=0.551)-exp(0.551^2/2))/sqrt(exp(0.551^2)*(exp(0.551^2)-1)) # has skew 2, standardised lognormal(0,0.551) (minus mean, divide by standard deviation)
...
# candidate_params <- list(list("X_eff" = seq(from=0.96,1,0.005), "n" = 50),
#                          list("X_eff" = seq(from=0.62,0.64,0.005), "n" = 100),
#                          list("X_eff" = seq(from=0.42,0.44,0.005), "n" = 200))
...
#     pow[i] <- logistic_regression_power_calc(n,Z_eff = Z_eff,X_eff = X_eff, B_small = B)
#   }
#   return(pow)
# }
# 
# t1 <- Sys.time()
# B_pl <- 10000
# power_list4 <- lapply(candidate_params,power_vec, B = B_pl)
# t2 <- Sys.time()
# difftime(t2,t1)
# plot_power_list <- function(power_list,candidate_params){
#   for (i in 1:length(power_list)){
#     power_i <- power_list[[i]]
#     X_eff_i <- candidate_params[[i]]$X_eff
...
#     abline(v=X_eff_pred)
#     print(paste0(i,": predicted X_eff = ", round(X_eff_pred,5)))
#   }
# }
# plot_power_list(power_list4,candidate_params)

n_obss <- c(50,100,200) # ,500,1000
print(n_obss)
# betas <- c(0.87,0.59,0.41) # for X ~ N(0,1)
betas <- c(0.98,0.636,0.425) # for X lognormal skew 2
